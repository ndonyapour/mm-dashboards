...
# Function to extract SMILES mapping from specific HTML files (vis_pose_sod1_onionnet_X.html)
def extract_smiles_mapping() -> Dict[str, str]:
    ...
    for file_name in os.listdir(assets_folder):
        ...
        # Ensure it matches the naming pattern "vis_pose_sod1_onionnet.html"
        if file_name.startswith("vis_pose_sod1_onionnet_") and file_name.endswith(".html") and os.path.isfile(file_path):
            try:
...
def Page():
    ...
    These docking poses were generated using **DiffDock** as part of the Virtual Screening workflow.  
    The compounds were extracted using the **NCATS HeliosAPI** and screened against  
    the **Human SOD1 G85R Variant (ODBID: 3CQP)**. OnionNet was then used to  
    calculate the binding free energy.  

    Select a SMILES below to view the corresponding docking visualization.
    """,
    ...
    if not smiles_options:
        solara.Text("No HTML files matching the pattern 'vis_pose_sod1_onionnet.html' found in the assets folder.")
        return
